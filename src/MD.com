...
! Energy Variables
	real E, ET, P1, KE, E_dbG, dihedE_dbG
...
! Variables used for contacts LJ
	...
	parameter (maxCon =25*Nmax)
	dimension sigma(maxCon), epsC(Maxcon)
	dimension IC(maxcon), JC(maxcon), trip(3,maxcon)


! Variables used for contacts dbG
        integer NC_dbG, IC_dbG, JC_dbG, ind_dbG
	   real Amp1, Mu1_array, Sig1_array,
     Q  Amp2, Mu2_array, Sig2_array, Rad_array
	   dimension Amp1(maxcon), Mu1_array(maxcon),Sig1_array(maxcon),
     Q  Amp2(maxcon), Mu2_array(maxcon), Sig2_array(maxcon),
     Q  Rad_array(maxcon), IC_dbG(maxCon), JC_dbG(maxCon)
! End of dbG variables
...
! Variables used for dihedrals dbG
        integer nPA_dbG, IP_dbG, JP_dbG, KP_dbG, LP_dbG, indP_dbG
        real Amp1Dihed, Mu1Dihed_array, Sig1Dihed_array,
     Q  Amp2Dihed, Mu2Dihed_array,
     Q  Sig2Dihed_array, RadDihed_array,
     Q  dbGPhaseDihed_array
	   dimension Amp1Dihed(Npmax), Mu1Dihed_array(Npmax),
     Q  Sig1Dihed_array(Npmax),
     Q  Amp2Dihed(Npmax), Mu2Dihed_array(Npmax),
     Q  Sig2Dihed_array(Npmax),
     Q  RadDihed_array(Npmax),
     Q  IP_dbG(Npmax), JP_dbG(Npmax),
     Q  KP_dbG(Npmax), LP_dbG(Npmax),
     Q  dbGPhaseDihed_array(Npmax*100)
! end
...
! conditional flags for additional execution settings 
     ...
     Q			  writeContactsRanges, isdbGaussian, isdbGaussian_Dihed,
     Q                minTrajOut, ExcludeStatsFromTraj, write3body
...
! addtional parameters, used only if conditional flag is set 
	...
	dimension boxMin(3),boxMax(3)
     ...
     Q ContactRangesEnergyFile, isdbGaussian, isdbGaussian_Dihed, minTrajOut,
     Q ExcludeStatsFromTraj, write3body
     ...
     Q EtotalRanges,
     Q Amp1, Mu1_array, Sig1_array,
     Q Amp2, Mu2_array, Sig2_array, Rad_array,
     Q Amp1Dihed, Mu1Dihed_array, Sig1Dihed_array,
     Q Amp2Dihed, Mu2Dihed_array,
     Q Sig2Dihed_array, RadDihed_array,
     Q dbGPhaseDihed_array
     ...
     Q NC_dbG, IC_dbG, JC_dbG, ind_dbG, nPA_dbG, IP_dbG, JP_dbG, 
     Q KP_dbG, LP_dbG
