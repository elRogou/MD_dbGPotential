...
!***********************************************************************
     ...
     Q  currentContactRange
        ...
        character(LEN=20) FMTB,FMTT,FMTP,FMTC, FMTCG,CA,RP,FMTE,FMTI,ERP,
     Q  FMTPG
        ...
        FMTI="(5I5,2F8.3)"
        FMTC="(5I5,4F8.3)"
        FMTCG ="(3I5,7F8.3)" ! formatting for reading the dual basin Gaussian
        FMTPG = "(5I5,7F8.3)"
        CA="(3I5,F10.3,F9.6)"
        RP="(I8,2I5,2F10.3)"
        ERP="(I8,3I5,2F10.3)"
...
! NOA - if there is no static atoms: instead of lastDynamicAtom = ANr
! (Original file),
...
! NOA - change the condition in order this will give random velocities
! only to Dynamic Protein atoms... 
        cnt = 1
...
555   continue
        !!! AMIT: entered this small loop in the 3 lines below to allow
        !using the startt=1 flag
        do i=1, ANr
            read(25,*)
        end do
...
! that was created by this program so there is position on one line and
! momenta
...
! in order to make the force and energy calculations easier, later.
       ...
       if (isdbGaussian_Dihed .eq. 'YES') then
         read(30,*) nPA_dbG
         if (nPA_dbG .gt. Npmax) then
            write(*,*) 'Too many dbG dihedrals'
            call abort
         endif
         
         ! write(*,*) 'reading dihedrals', nPA_dbG
         do i = 1, nPA_dbG
          ! write(*,*) i
          read(30, FMTPG) indp_dbG, IP_dbG(i), JP_dbG(i),
     Q                    KP_dbG(i), LP_dbG(i), Amp1Dihed(i),
     Q                    Mu1Dihed_array(i),
     Q                    Sig1Dihed_array(i), Amp2Dihed(i),
     Q                    Mu2Dihed_array(i), 
     Q                    Sig2Dihed_array(i), RadDihed_array(i),
     Q                    dbGPhaseDihed_array(i)
          enddo
       endif
...
! READ DUAL BASIN GAUSSIAN PAIRS
        ! write(*,*) 'checking gaussian flag ', isdbGaussian
        if (isdbGaussian .eq. 'YES') then  
            ! write(*,*) 'Reading the number of gaussians'
            read(30,*) NC_dbG

                if (NC_dbG .gt. MaxCon) then 
                    write(*,*) 'The number of gaussian contacts exceeds max'
                    call abort
                endif
            ! write(*,*) 'Reading gaussian contacts'
            do i = 1, NC_dbG
                read(30, FMTCG) ind_dbG, IC_dbG(i), JC_dbG(i), Amp1(i),
     Q                          Mu1_array(i),
     Q                          Sig1_array(i), Amp2(i), Mu2_array(i), 
     Q                          Sig2_array(i), Rad_array(i)
            enddo
        endif
...
!READ ELECTROSTATICS
     ...
     Q    tempCharge(esaIter)
        ...
        read(30,*) AN
! NOA - if there is no static atoms: instead of lastDynamicAtom = AN
! (Original file),
...
! These are safeguard statements
     ...
     Q,' AN ',AN
        ...
        if(Trajectory .ne. 'NO')then
          if (ExcludeStatsFromTraj .ne. 'YES') then
            if(minTrajOut .ne. 'YES')then
              write(6,*) MDT
            else
               write(6,*) AN, stepstop, WOT
            endif
          endif
        endif
        ...
        if(Trajectory .ne. 'NO')then
          ...
          ! else
          !   write(6,*) chainlength(i), stepstop, WOT
          endif
        endif
      enddo
      ...
      if (ExcludeStatsFromTraj .eq. 'YES') then
        ...
        if (minTrajOut .eq. 'YES') then
          write(6,*) DynamicAtomRange(DynLength), stepstop, WOT
        endif
      endif
     ...
     Q  ms(i)
        ...
        if(Trajectory .ne. 'NO')then
          if(minTrajOut .ne. 'YES')then
          if (i .lt. (DynamicAtomRange(DynLength)+1)) then
...
! NOA - if both atom static the interaction is between 2 static atoms
! and is therfore neglected
              isDynFirst = 
