...
!***********************************************************************
  ...
  !      read(60,*) AN ! number of atoms in simulation
        ...
        read(60,*) isdbGaussian ! flag for the use of dbgaussian potential
        read(60,*) isdbGaussian_Dihed ! flag for the use of dbg on dihedrals
        ! write(*,*) 'after reading the gauss flag: ', isdbGaussian
        read(60,*) minTrajOut ! This flag when equals YES activates the output 
        ! of minimal Traj file: the minimal file does not contain any information
        ! besides the x y z positions. Basically you will get a large chunk of data
        ! comprised of three columns and the total number of beads as the header.
        read(60,*) write3body !whether to calculate and write three-body interactions
...
! check whether to write 2 Body & Elj for selected Contact Ranges
     ...
     Q    status='unknown',access='sequential')
        ...
        if(hasStaticAtoms .eq. 'YES')then
          ...
          read(60,*) ExcludeStatsFromTraj   ! wheter to write static atoms coordinates to trajectory
        endif
