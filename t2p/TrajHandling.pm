...
use base 'Exporter';
our @EXPORT_OK = qw(readBeadsData readResiduesAndBeadsData generatePDBResiduesArray readFrame);
...
sub readBeadsData
{
    ...
    for (my $beadIter = 0; $beadIter < $numberOfBeads; $beadIter++)
    {
      ...
      if (exists($beadTypesRef->{$beadType}) or ($currentResidueType ne $residueType) or ($currentDNAResidueType ne $residueType))
      {
